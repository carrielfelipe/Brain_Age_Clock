...
class Brain_Model:
    def __init__(self,save_path=None,  params_space=None, fit_params_search=None, model_params_search=None, name_model=None):
    ...
    def search_best_model(self, X=None, y=None, param_space_=None, n_iter_=10, n_jobs_=-1, scoring_metric='neg_mean_absolute_error', type_model=1):
        ...
        if param_space_ is None:
            param_space = self.model_params_search
        else:
        ...
        # Instanciar el modelo dependiendo del tipo (tipo 1 o tipo 2)
        if type_model == 1:
            model = self.model_ml(**self.model_params_search)
        elif type_model == 2:
            model = self.model_ml

        # Establecer el BayesSearchCV para la optimización
        ...
        # Retornar el mejor modelo y los mejores parámetros encontrados
    ...
    def calculate_multiple_shap(self, df_train, df_test,ID_label, results_per_fold_train, results_per_fold_test, models_list, feature_col_range, iteration=20, kfolds_=10,  scaler=2, random_seed=42):
    ...
    def normalized_region(self, shap_summary_sorted, num_max=20):
...
class BaseRegressor(Brain_Model):
    def __init__(self,save_path=None, params=None, params_space=None, fit_params_search=None, model_params_search=None,fit_params_train=None, model_params_train=None, name_model=None):             
        super().__init__(save_path, params_space, fit_params_search, model_params_search, name_model)
        ...
        #self.model_params_search = model_params_search if model_params_search is not None else {}
        self.fit_params_train = fit_params_train if fit_params_train is not None else {}
        self.model_params_train = model_params_train if model_params_train is not None else {}
    ...
    def test(self, X , y , ID, n_splits=10, n_iterations=20, scaler=2, result_model= None):

        results_per_fold_test = []
        # Inicializar resultados  
        
        results_labels_df_test = pd.DataFrame(columns=['y_labels', 'y_pred', 'y_pred_corrected', 'GAP', 'GAP_corrected', 'ID-unique'])
        ...
        return results_labels_df_test, results_per_fold_test